...
public class TextAlignmentAction extends SelectionAction {
    ...
    private List<ITextAlignment> getValidSelectedObjects() {
        ...
        for(Object o : getSelectedObjects()) {
            if(o instanceof EditPart editPart) {
                ...
                if(shouldEnable(object)) {
                    list.add((ITextAlignment)object);
                }
            }
        }
    ...
    private boolean shouldEnable(Object object) {
        if(object instanceof ILockable lockable && lockable.isLocked()) {
            return false;
        }
        
        if(object instanceof ITextAlignment ta) {
            IObjectUIProvider provider = ObjectUIFactory.INSTANCE.getProvider(ta);
