        ...
        CompoundCommand compoundCommand = new AlwaysExecutingCompoundCommand(TEXT) {
            ...
            public void execute() {
                for(IDiagramModelObject dmo : getObjectsToBeDeleted()) {
                    ...
                    // Delete main object
                    Command cmd = DiagramCommandFactory.createDeleteDiagramObjectNoChildrenCommand(dmo);
                    add(cmd);
                    cmd.execute();
                    ...
                    for(IDiagramModelObject child : new ArrayList<>(children)) {
                        ...
                        cmd = new RemoveListMemberCommand<>(children, child);
                        add(cmd);
                        cmd.execute();
                        
                        // Adjust x,y position to new parent
                        cmd = new ChangePositionCommand(child, dmo.getBounds());
                        add(cmd);
                        cmd.execute();
