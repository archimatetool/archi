...
public class SearchWidget extends Composite {
	...
	private IAction fActionFilterPropertiesKeysValues;

	private MenuManager fPropertiesMenu;

	private List<IAction> fObjectActions = new ArrayList<IAction>();

	// Hook into the global edit Action Handlers and null them when the text
	...
	protected void setupSearchTextWidget() {
		if (PlatformUtils.isWindows()) {
			...
			fSearchText = new Text(this, SWT.SEARCH | SWT.ICON_CANCEL
					| SWT.ICON_SEARCH);
	...
	protected void setupToolBar() {
		fActionFilterName = new Action(Messages.SearchWidget_0,
		...
		fActionFilterPropertiesKeysValues = new Action(Messages.SearchWidget_14,
				IAction.AS_CHECK_BOX) {
			@Override
			public void run() {
				fSearchFilter.setFilterOnPropertiesKeysValues(isChecked());
			};
		};

		fActionFilterPropertiesKeysValues.setToolTipText(Messages.SearchWidget_14);
		fActionFilterPropertiesKeysValues.setChecked(true);
		fSearchFilter.setFilterOnPropertiesKeysValues(true);

		
		fActionFilterDoc = new Action(Messages.SearchWidget_2,
				IAction.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		AbstractDropDownAction dropDownAction = new AbstractDropDownAction(
				Messages.SearchWidget_4) {
			@Override
			public void run() {
			...
			public ImageDescriptor getImageDescriptor() {
				return IArchimateImages.ImageFactory
						.getImageDescriptor(IArchimateImages.ICON_FILTER_16);
			}
		};
		toolBarmanager.add(dropDownAction);

		// Name & Documentation & keyvalue
		dropDownAction.add(fActionFilterName);
		dropDownAction.add(fActionFilterDoc);
		dropDownAction.add(fActionFilterPropertiesKeysValues);
		...
		MenuManager implementationMenu = new MenuManager(
				Messages.SearchWidget_10);
		dropDownAction.add(implementationMenu);
		for (EClass eClass : ArchimateModelUtils
				.getImplementationMigrationClasses()) {
		...
		IAction action = new Action(Messages.SearchWidget_12,
				IAction.AS_CHECK_BOX) {
			@Override
			public void run() {
	...
	private IAction createObjectAction(final EClass eClass) {
		IAction action = new Action(
			...
			@Override
			public ImageDescriptor getImageDescriptor() {
				return ArchimateLabelProvider.INSTANCE
						.getImageDescriptor(eClass);
			}
		};
	...
	private void getAllUniquePropertyKeysForModel(IArchimateModel model,
			List<String> list) {
