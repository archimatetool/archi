...
public class TreeModelView extends AbstractModelView implements ITreeModelView, IUIRequestListener {
	...
	private List<IViewerAction> fActionsChangeElementType;
	...
	private void makeActions() {
		...
		fActionsChangeElementType = ChangeElementTypeAction.makeActions(getViewer());
	...
	private void registerGlobalActions() {
		...
		// ArchiActionFactory.CHANGE_ELEMENT_TYPES is not initialized yet
		for (ActionFactory actionFactory : ChangeElementTypeAction.getActionFactories().keySet()) {
			actionBars.setGlobalActionHandler(actionFactory.getId(), ChangeElementTypeAction.getActionFactories().get(actionFactory));	
		}
	...
	/**
	 * Disable Global Actions We need to do this when closing the View
	 */
	...
	private void fillContextMenu(IMenuManager manager) {
		...
		if (!isEmpty) {
			...
			IMenuManager refactorMenu = new MenuManager(com.archimatetool.editor.diagram.Messages.AbstractDiagramEditorActionBarContributor_5bis,
					"treemenu_refactor"); //$NON-NLS-1$
			manager.add(refactorMenu);
			ChangeElementTypeAction.fillMenu(refactorMenu);
	...
	private void updateActions() {
		...
		fActionsChangeElementType.stream().forEach((a) -> a.update());
