...
public class CompositeMultiImageDescriptor extends CompositeImageDescriptor {
    ...
    protected void drawCompositeImage(int width, int height) {
        for(ImageDescriptor overlay : fOverlays) {
            if(overlay != null) {
                drawImage(zoom -> overlay.getImageData(ImageFactory.getDeviceZoom()), 0, 0); // Not sure how this works
            }
        }
    }
    ...
    protected Point getSize() {
        ...
        if(fSize == null) {
            ...
            for(ImageDescriptor overlay : fOverlays) {
                if(overlay != null) {
                    ImageData id = overlay.getImageData(ImageFactory.getDeviceZoom());
